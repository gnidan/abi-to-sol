...
export const Type: fc.Memo<string> = fc.memo((n) =>
  n === 0
    ? Primitive()
...
export const FunctionName = () => fakerToArb("{{hacker.verb}} {{hacker.noun}}");

export const TypeRecord = (): fc.Arbitrary<any> =>
  Type().chain((type) =>
...
export const NormalFunction = () =>
  fc
    ...
    .map((records) => records.reduce((a, b) => ({...a, ...b}), {}))
    .filter((entry) => {
      const {inputs, outputs = []} = entry as Types.FunctionAbiEntry;
      // names that are not blank should be unique
...
export const ConstructorFunction = () =>
  fc
    .tuple(
      fc.record({
...
export const Abi = (): fc.Arbitrary<Types.Abi> =>
  fc
    .tuple(
      ConstructorFunction(),
      FallbackFunction(),
      ReceiveFunction(),
      fc.array(fc.oneof(NormalFunction(), Event()))
    )
    .chain(([constructor, fallback, receive, entries]) =>
      fc.shuffledSubarray([constructor, fallback, receive, ...entries])
    ) as fc.Arbitrary<Types.Abi>;
